...
static int convert(lua_State *L, unsigned (*conv)(unsigned)) {
  int t = lua_type(L, 1);
  if (t == LUA_TNUMBER)
    lua_pushinteger(L, conv((int) lua_tointeger(L, 1)));
...
static int Lutf8_width(lua_State *L) {
  ...
  size_t default_width = luaL_optinteger(L, 3, 0);
  if (t == LUA_TNUMBER) {
    size_t chwidth = utf8_width((int)lua_tointeger(L, 1), ambi_is_single);
...
static int Lutf8_widthindex(lua_State *L) {
  ...
  size_t default_width = luaL_optinteger(L, 4, 0);
  size_t idx = 1;
  while (s < e) {
    unsigned ch;
    size_t chwidth;
...
LUALIB_API int luaopen_plugin_utf8(lua_State *L) {
  luaL_Reg libs[] = {
